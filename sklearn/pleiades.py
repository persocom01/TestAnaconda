...
class CZ:
    ...
    def csv_insert(self, file, updatekey=None, postgre=False, tablename=None, chunksize=None, sizelim=1073741824, printable=False, **kwargs):
        ...
        def alchemy_insert(df, updatekey=None, tablename=None):
            try:
                df.to_sql(tablename, self.cursor,
            ...
            if isinstance(pkeys, str):
                ...
                if tablename:
                    for i, file in enumerate(files):
                        ...
                        if i == 0:
                            self.csv_table(
                                files[i], tablename=tablename, updatekey=pkeys[i], **kwargs)
                        try:
                            self.csv_insert(
                                file, tablename=tablename, updatekey=pkeys[i], **kwargs)
                        except TypeError or IndexError:
                            self.csv_insert(
                                file, tablename=tablename, **kwargs)
                else:
                    for i, file in enumerate(files):
                        if clean_colnames:
                            self.csv_clean_colnames(file)
